...
def test_context_update_upstream_services():
    ...
    def assert_upstream_services(c, e):
        ...
        s = Span(tracer=None, name="assertion")
        assert s.meta == {}
        c._update_tags(s)
        assert s.meta[UPSTREAM_SERVICES_KEY] == e
    ...
    s1 = Span(tracer=None, name="test", service="example")
    ...
    assert_upstream_services(c1.clone(), "ZXhhbXBsZQ==|2|3|0.6570")

    # When we update the upstream service multiple times
    #   - We only keep the most recent entry
    s2 = Span(tracer=None, name="test", service="test-service")
    ...
    assert_upstream_services(c1.clone(), "dGVzdC1zZXJ2aWNl|1|0|1.0000")
    ...
    assert_upstream_services(c3.clone(), inherited)

    # Updating our entry when we have an inherited entry
    #   - We do not remove the upstream entry
    #   - We append our entry
    s3 = Span(tracer=None, name="test", service="my-service")
    ...
    assert_upstream_services(c3.clone(), "{};bXktc2VydmljZQ==|2|4|".format(inherited))

    # Updating our entry multiple times when we have an inherited entry
    #   - We do not remove the upstream entry
    #   - We only keep our most recent entry active
    s3 = Span(tracer=None, name="test", service="another-service")
    ...
    assert_upstream_services(c3.clone(), "{};YW5vdGhlci1zZXJ2aWNl|2|0|0.5000".format(inherited))
